...
namespace InnsmouthCafe.UI
{
    ...
    public class ModeSelectPanelUI : MonoBehaviour
    {
        ...
        [Header("锁图标")]
        [SerializeField] [Tooltip("新手模式锁图标（未解锁时显示）")]
        private GameObject _beginnerLock;

        [SerializeField] [Tooltip("普通模式锁图标（未解锁时显示）")]
        private GameObject _normalLock;

        [Header("按钮文本")]
        [SerializeField] [Tooltip("新手模式按钮文本")]
        private TextMeshProUGUI _beginnerText;

        [SerializeField] [Tooltip("普通模式按钮文本")]
        private TextMeshProUGUI _normalText;

        [Header("模式配置")]
        [SerializeField] [Tooltip("新手模式配置SO")]
        private GameModeConfigSO _beginnerConfig;
        ...
        private void Awake()
        {
            ...
            _beginnerButton?.onClick.AddListener(OnBeginnerClicked);
        ...
        {
            bool beginnerUnlocked = true; // 教学完成后才能进入此面板，新手必定解锁
            bool normalUnlocked = false;

            if (GameManager.Instance != null)
            {
                beginnerUnlocked = GameManager.Instance.IsModeUnlocked(GameMode.Beginner);
                normalUnlocked = GameManager.Instance.IsModeUnlocked(GameMode.Normal);
            }

            SetButtonState(_beginnerButton, _beginnerLock, _beginnerText, beginnerUnlocked);
        ...
        {
            if (_beginnerConfig == null)
            {
                Debug.LogError("[ModeSelect] 新手模式配置未设置");
                return;
            }

            GameManager.Instance?.StartGameWithConfig(_beginnerConfig);
        }
