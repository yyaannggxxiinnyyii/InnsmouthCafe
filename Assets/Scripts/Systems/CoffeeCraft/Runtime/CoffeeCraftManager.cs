...
public class CoffeeCraftManager : Singleton<CoffeeCraftManager>
{
    ...
    private OrderRequirementData _currentOrder;
    ...
    public event Action<LiquidSO> OnPouringChanged; // 开始/停止倒液时触发，参数为当前倒入的液体（停止时为null）
    ...
    public void SelectCup(CupContainerData cup)
    {
        ...
        OnModuleStateChanged?.Invoke(_moduleState);
        TutorialEventBus.Publish(TutorialEvents.CupSelected);
    ...
    public void AddBean(BeanSO bean)
    {
        ...
        OnModuleStateChanged?.Invoke(_moduleState);
        TutorialEventBus.Publish(TutorialEvents.BeanAdded);
    ...
    public void SelectGrind(GrindType grindType)
    {
        ...
        OnBatchDataChanged?.Invoke(_currentBatch);
        OnModuleStateChanged?.Invoke(_moduleState);
        TutorialEventBus.Publish(TutorialEvents.GrindSelected);
    ...
    public void StartExtraction()
    {
        ...
        TutorialEventBus.Publish(TutorialEvents.ExtractionStarted);
    ...
    private void FinishExtraction()
    {
        ...
        TutorialEventBus.Publish(TutorialEvents.ExtractionCompleted);
    ...
    public void StartPourLiquid(LiquidSO liquid)
    {
        ...
        if (IngredientUnlockManager.Instance != null && !IngredientUnlockManager.Instance.IsLiquidUnlocked(liquid))
        {
            ActionLogBus.Log("该辅助液尚未解锁", new Color(1f, 0.6f, 0f));
            Debug.LogWarning($"[CoffeeCraft] 辅助液尚未解锁：{liquid.liquidName}");
            return;
        }
        ...
        _isPouring = true;
        OnPouringChanged?.Invoke(liquid);
    ...
    public void StopPourLiquid()
    {
        ...
        TutorialEventBus.Publish(TutorialEvents.LiquidAdded);
    ...
    public void AddTopping(ToppingSO topping, Vector2 localPosition)
    {
        ...
        if (IngredientUnlockManager.Instance != null && !IngredientUnlockManager.Instance.IsToppingUnlocked(topping))
        {
            ActionLogBus.Log("该小料尚未解锁", new Color(1f, 0.6f, 0f));
            Debug.LogWarning($"[CoffeeCraft] 小料尚未解锁：{topping.toppingName}");
            return;
        }
        ...
        OnCoffeeDataChanged?.Invoke(_currentCoffeeData);
        TutorialEventBus.Publish(TutorialEvents.ToppingAdded);
    ...
    private static string GrindTypeToName(GrindType grindType)
    {
        ...
        {
            GrindType.Coarse => "低精度研磨",
            GrindType.Fine => "中精度研磨",
            GrindType.ExtraFine => "高精度研磨",
