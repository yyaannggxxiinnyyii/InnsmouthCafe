...
namespace InnsmouthCafe.UI
{
    ...
    public class EndingPanelUI : MonoBehaviour
    {
        ...
        /// <summary>
        /// 从图鉴中重播指定结局；播放结束后关闭结局层并留在当前界面。
        /// </summary>
        public void ReplayEndingFromGallery(GameEnding ending)
        {
            if (_endingConfigSO == null)
            {
                Debug.LogError("[Ending] EndingConfigSO 未配置，无法从图鉴重播结局");
                return;
            }
            ...
            if (_currentConfig == null)
            {
                Debug.LogError($"[Ending] 图鉴重播结局配置为空: {ending}");
                return;
            }
        ...
        private IEnumerator PlayEndingSequence()
        {
            ...
            {
                _endingClickButton.interactable = true;
            }
        ...
        private void ShowEndingPanel()
        {
            ...
            if (_endingImage != null && _currentConfig.endingImage != null)
            {
                ...
                _endingImage.color  = _currentConfig.endingImageColor;
            }

            // 设置标题颜色（文字内容由打字机填充）
            if (_endingTitleText != null)
            {
                _endingTitleText.color = _currentConfig.endingTitleColor;
        ...
        private IEnumerator FadeOutToMainMenu()
        {
            ...
            // 切回默认BGM，停止非主轨道
            AudioManager.Instance?.StopSecondaryBgms();
        ...
        /// 关闭图鉴重播结局层，并恢复默认BGM。
        /// </summary>
        private void CloseGalleryReplay()
        {
            HideEndingLayer(true);
        ...
        /// <summary>
        /// 隐藏完整结局播放层，并重置运行时状态，避免跨场景保留后继续遮挡主菜单。
        /// </summary>
        /// <param name="stopCoroutines">是否停止当前结局播放协程。</param>
        private void HideEndingLayer(bool stopCoroutines)
        {
            if (stopCoroutines)
            {
                StopAllCoroutines();
            ...
            KillGroupTween(_rootCanvasGroup);
            KillGroupTween(_blackOverlay);
            KillGroupTween(_cutscenePanel);
            KillGroupTween(_endingPanel);
        ...
        /// <summary>
        /// 停止指定 CanvasGroup 上未完成的渐隐渐显动画。
        /// </summary>
        /// <param name="group">需要停止动画的 CanvasGroup。</param>
        private void KillGroupTween(CanvasGroup group)
        {
            if (group == null)
            {
                return;
